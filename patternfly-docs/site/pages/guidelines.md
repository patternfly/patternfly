...
PatternFly follows a two-layer theming system where **CSS tokens** always inform **component variables**. Each one of those layers follow a set of very specific rules.

#### CSS tokens

The main reason to have CSS tokens is to maintain consistency across the design system. They adhere to the following rules:
...
For example a CSS token setup would look like:
...
- The value of a component variable is **always** defined by a CSS token.
...
```scss
// Component scoped variables are always defined by CSS tokens
...
If a situation arises where a value needs entering into the style sheets that isn't already defined in the CSS tokens this should serve as a red flag to you.
...
.pf-nav {
  ...
  @media (min-width: var(--pf-t--global--breakpoint--sm)) {
    // non-mobile styles
  }
}
```
...
We create CSS tokens to maintain consistency across the design system. These values also inform our component level variables.
