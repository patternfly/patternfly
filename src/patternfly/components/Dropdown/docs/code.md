...
| `.pf-m-disabled` | `div.pf-c-dropdown__toggle.pf-m-split-button` | Modifies to display the dropdown toggle as disabled. This applies to `div.pf-c-dropdown__toggle.pf-m-split-button` and should not be used in lieu of the `disabled` attribute on `button.pf-c-dropdown__toggle`, `.pf-c-dropdown__toggle-button`, or `.pf-c-dropdown__toggle-check > input[type="checkbox"]`|
