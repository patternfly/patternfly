...
{{#> check}}
  ...
  {{#> check-label check-label--attribute='for="check-required"' required="true"}}Check{{/check-label}}
{{/check}}
```
...
| `required` | `<input type="checkbox">` | Indicates that the element is required. |
